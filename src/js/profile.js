...
const fetchUser = ({ username, options }) =>
	fetch(auth.API_URL + `/api/users/${username}`, options).then((res) => res.json());

const fetchAllUsers = ({ options }) =>
	fetch(auth.API_URL + `/api/users/`, options).then((res) => res.json());

const fetchAllPosts = ({ query, options }) =>
	fetch(auth.API_URL + `/api/posts/${query}`, options).then((res) => res.json());
